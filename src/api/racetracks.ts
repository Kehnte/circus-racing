...
router.post("/", ...requireModo, async (req, res) => {
  ...
  if (!name || !Array.isArray(checkpoints) || checkpoints.length < 2) {
    res.status(400).json({ error: "name and at least 2 checkpoints are required" });
    return;
  }
  const normalized = checkpoints.map((cp: any, i: number) => ({
    order: i,
